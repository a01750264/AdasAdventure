...
public class CamaraLalo : MonoBehaviour
{
    ...
    void Update()
    {
        float x = Mathf.Clamp(personaje.transform.position.x, min: 0, max: 4);   // Establecemos los limites en el eje x
        float y = Mathf.Clamp(personaje.transform.position.y, min: 0, max: 80);  // Establecemos los limites en el eje y
